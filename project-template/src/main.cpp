...
int main(int argc, char **argv) {
  if (argc == 3) {
    try {
      ...
      // std::stoi throws std::invalid_argument or std::out_of_range
      // if conversion fails; we catch std::exception and return a
      // non-zero, documented exit code.
  ...
  // If the user did not provide the required arguments, print a short
  // usage message. This makes the program easy to use in manual testing
  // from the terminal and from automated scripts.
